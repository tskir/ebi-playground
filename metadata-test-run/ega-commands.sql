...
-- Mouse analyses in EGA
SELECT ANALYSIS_ID
FROM (
  SELECT EXTRACTVALUE(ANALYSIS_XML, '/ANALYSIS_SET/ANALYSIS/ANALYSIS_TYPE/*/ASSEMBLY/STANDARD/@accession') AS ACCESSION, ANALYSIS_ID
  FROM ANALYSIS
  WHERE ANALYSIS_TYPE IN ('PROCESSED_READS', 'REFERENCE_ALIGNMENT', 'SEQUENCE_VARIATION')
  AND EGA_ID IS NOT NULL
)
WHERE ACCESSION = 'GCA_000001635.2'
